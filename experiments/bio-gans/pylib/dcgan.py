...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        x = torch.nn.functional.leaky_relu(self.conv1(x), 0.2, True)
        x = torch.nn.functional.leaky_relu(self.bn2(self.conv2(x)), 0.2, True)
        x = torch.nn.functional.leaky_relu(self.bn3(self.conv3(x)), 0.2, True)
        x = torch.nn.functional.leaky_relu(self.bn4(self.conv4(x)), 0.2, True)
