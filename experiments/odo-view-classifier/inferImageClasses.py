...
def inferImageClasses(image_ids, infer_colname, infer_names, model_location, image_size, show):
    ...
    for i in range(0,len(image_ids)):
        image_ids[i] = "../../data/all_images/" + image_ids[i] + ".jpg"
    ...
    for img_loc in image_locs:
        ...
        print(a)
        print(torch.max(a))
        max, index = torch.max(a,dim=0)
        print(index)
        inferences.append(index)
        # return the label
    ...
    inferences = pd.DataFrame({'imageID': file_list, infer_colname: inferences})

    # fix imageIDs by removing .jpg
    inferences["imageID"].replace(".jpg", "", regex=True)
