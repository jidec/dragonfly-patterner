...
# get ids for a genus, not in train data
...
inferImageClasses(image_ids=ids, infer_colname="dorsal_lateral_bad", infer_names=["bad","dorsal","dorsolateral","lateral"],
                  model_name="4-class", print_steps=True,show=True)
#ids = getFilterImageIDs(infer_fields=["dorsal_lateral_bad"],infer_values=["dorsal"])

# 2_segmentations
# infer segment masks and save to data/masks
inferSegments(ids,model_name='segmenter_randaug6',increase_contrast=True,activation_threshold=0.6,print_steps=True,show=True)

# extract segments using masks, modifying the raw masks according to params
extractHoneSegments(ids,bound=True, remove_islands=True, set_nonwhite_to_black=True,erode_kernel_size=0,print_steps=True,show=True,write=True)

ids = filterIDsWithComponents(ids,"segment")

# 3_discretization_compositing
colorDiscretize(ids,by_contours=False,print_details=True,show=True) #group_cluster_records_col="species")
