...
# We expect:
# higher pigment proportions in colder environments 
# males should have more pigments due to their increased selection for territoriality
#   NOTE - could get an expert to identify the most male-territorial clades, and see if this pattern holds more in these clades
# dragonflies that overlap in their ranges should have different patterns (when correcting for effect of shared enviroment)
#   NOTE - this should occur stronger when species share a habitat preference
# there should be a phylogenetic effect - closely related species should be more similar in color
# individuals within a species should be similar in color, but may also vary by their enviroments 
# the proportions of brown and black pigments should be correlated phylogenetically - in clades where one evolves another is likely to evolve 
# the placement of pigments should be correlated due to conserved patterning processes
...
# 
taxa <- wings[!duplicated(wings$Species.name),]
taxa <- dplyr::select(taxa, c("Suborder","Family","Genus","Species","Species.name","North.America","South.America","Central.America"))
clipr::write_clip(taxa) 
...
#basic lmer 
library(lme4)
wings$Species <- as.factor(wings$Species)
wings$Sex <- as.factor(wings$Sex)
m <- lmer(col_6_prop ~ meanTemp + sdTemp + (1 + Sex | Species),data=wings)
summary(m)

?lmer
library(phyr)

test <- pglmm(bby_total ~ 0 + flight_type + (1 | species__),
                  data = wings_sp, cov_ranef = list(species=phy),bayes=T)
