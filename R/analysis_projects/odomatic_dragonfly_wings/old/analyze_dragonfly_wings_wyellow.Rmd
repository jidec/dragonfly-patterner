...
Formula for black: is "flight_type + wing_type + Sex + temp_sd_sp + temp_q95_sp + se_indv + prec_indv + prec_mean_sp + fore_length"
...
## Q1: Are patterns with pigment at the base of the wing associated with climate?
...
```{r base_pigs, results='hide'}

wings_sp$temp_mean_indv
m <- lm("PC2_foreallcut ~ temp_indv * wing_area",data=wings)
summary(m)
m <- lm("PC2_foreall ~ temp_indv * wing_area",data=wings)
summary(m)
m <- lm("PC2_hindallcut ~ temp_indv * wing_area",data=wings)
summary(m)
m <- lm("PC2_hindall ~ temp_indv * wing_area",data=wings)
summary(m)

m <- lm("PC1_foreallcut ~ temp_indv * wing_area",data=wings)
summary(m)
m <- lm("PC1_foreall ~ temp_indv * wing_area",data=wings)
summary(m)
m <- lm("PC1_hindallcut ~ temp_indv * wing_area",data=wings)
summary(m)
m <- lm("PC1_hindall ~ temp_indv * wing_area",data=wings)
summary(m)


m <- lm("PC1_foreblack ~ temp_indv * wing_area",data=wings)
summary(m)
m <- lm("PC2_foreblack ~ temp_indv * wing_area",data=wings)
summary(m)
m <- lm("PC1_hindblack ~ temp_indv * wing_area",data=wings)
summary(m)
m <- lm("PC2_hindblack ~ temp_indv * wing_area",data=wings)
summary(m)
...
m <- lm("PC1_foreallcut ~ temp_indv * wing_area",data=wings_sp)
summary(m)
m <- lm("PC1_foreall ~ temp_indv * wing_area",data=wings_sp)
summary(m)
m <- lm("PC1_hindallcut ~ temp_indv * wing_area",data=wings_sp)
summary(m)
m <- lm("PC1_hindall ~ temp_indv * wing_area",data=wings_sp)
summary(m)

m <- lm("cut_fore_pc2 ~ temp_mean_sp",data=wings_sp)
summary(m)
m <- lm("cut_hind_pc2 ~ temp_mean_indv + temp_mean_sp",data=wings_sp)
summary(m)
m <- lm("black_fore_pc2 ~ temp_mean_indv + temp_mean_sp",data=wings_sp)
su
m <- lm("black_hind_pc2 ~ temp_mean_indv + temp_mean_sp",data=wings_sp)
summary(m)
```

No strong temp relationship, but relatively phylo conserved

## Q2: Are patterns phylo conserved?
...
assessTraitsOnTree(wings_sp_na0,trait_colname="black_fore_pc1",tree_location="../../data/odonata.tre")
assessTraitsOnTree(wings_sp_na0,trait_colname="black_hind_pc1",tree_location="../../data/odonata.tre")

assessTraitsOnTree(wings_sp_na0,trait_colname="black_fore_pc2",tree_location="../../data/odonata.tre")
assessTraitsOnTree(wings_sp_na0,trait_colname="black_hind_pc2",tree_location="../../data/odonata.tre")
...
## Q10: Do the big paper effects hold in Libullids or rm Calops?

```{r logregs, results='hide'}

summary(wings_lib$wing_area)

# model for interaction between wing_area and temperature for black, then brown
m <- getPlotLM(wings_lib,model_type="glmmTMB_beta_zi",response="black",
               formula="wing_area * temp_indv + (1 | species)")
plot_predictions(m, type="conditional", condition = list("wing_area","temp_indv")) + xlab("Scaled wing area") + ylab("Proportion of black color cluster in wing") + ylim(0,0.4)

# model for interaction between wing_area and temperature for black, then brown
m <- getPlotLM(wings_rm_cal,model_type="glmmTMB_beta_zi",response="black",
               formula="wing_area * temp_indv + (1 | species)")
plot_predictions(m, type="conditional", condition = list("wing_area","temp_indv")) + xlab("Scaled wing area") + ylab("Proportion of black color cluster in wing") + ylim(0,0.4)
...
Perchers are still dimorphic in different ways than flyers, but larger + darker = colder is no longer

Seems like the signal is partially driven by Calopyerygidae, interesting!
